...
public class ApplicationUninstallationAction implements Action {
	private static final Logger LOGGER = Logger
			.getLogger(ApplicationUninstallationAction.class.getName());

	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "Application uninstallation action");
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		String applicationId = (String) input.get("applicationID");
		try {
			execInterfacer.uninstallApplication(applicationId);
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put("uninstalled", true);
	}

	public void run(){
		try{
			LOGGER.log(Level.INFO, "Application uninstallation action thread");
