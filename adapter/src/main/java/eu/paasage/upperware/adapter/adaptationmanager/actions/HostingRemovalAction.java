...
public class HostingRemovalAction implements Action {

	private static final Logger LOGGER = Logger
			.getLogger(HostingRemovalAction.class.getName());
	HostingInstance hosting;

	public HostingRemovalAction(HostingInstance hosting) {
	...
	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "Hosting removal for " + hosting.getName());
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		try {
			execInterfacer.removeHosting(hosting.getName());
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put(hosting.getName(),"Removed");
	}
	
	public void run(){
		try{
			LOGGER.log(Level.INFO, "Hosting removal thread for " + hosting.getName());
