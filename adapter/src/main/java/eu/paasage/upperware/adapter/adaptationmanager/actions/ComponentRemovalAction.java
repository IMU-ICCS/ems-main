...
public class ComponentRemovalAction implements Action {

	private static final Logger LOGGER = Logger
			.getLogger(ComponentRemovalAction.class.getName());
	InternalComponentInstance c;

	public ComponentRemovalAction(InternalComponentInstance c) {
		this.c = c;
	}

	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "Component removal for " + c.getName());
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		try {
			execInterfacer.removeComponent(c.getName());
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put(c.getName(),"Removed");
	}
	
	public void run(){
		try{
			LOGGER.log(Level.INFO, "Component removal thread for " + c.getName());
