...
public class CommunicationAdditionAction implements Action {
	...
	private static final Logger LOGGER = Logger
			.getLogger(CommunicationAdditionAction.class.getName());
	CommunicationInstance cmm;

	public CommunicationAdditionAction(CommunicationInstance cmm) {
		this.cmm = cmm;
	}
	...
	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "Communication addition for " + cmm.getName());
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		String cmmId=null;
		try {
			cmmId=execInterfacer.addCommunication(cmm.getName());
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put(cmm.getName(),cmmId);

	}
	public void run(){
		try{
			LOGGER.log(Level.INFO, "Communication addition thread for " + cmm.getName());
			...
			execInterfacer.addCommunication(depParam);
		} catch(Exception e){
			try {
				throw new ActionError();
