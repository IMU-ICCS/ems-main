...
public class VMAdditionAction implements Action {
	...
	private static final Logger LOGGER = Logger
			.getLogger(VMAdditionAction.class.getName());
	VMInstance vm;

	public VMAdditionAction(VMInstance vm) {
		this.vm = vm;
	}

	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "VM addition for " + vm.getName());
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		String vmId=null;
		try {
			vmId = execInterfacer.addVM(vm.getName());
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put(vm.getName(), vmId);
	}
	
	public void run(){
		try{
			LOGGER.log(Level.INFO, "VM addition thread for " + vm.getName());
