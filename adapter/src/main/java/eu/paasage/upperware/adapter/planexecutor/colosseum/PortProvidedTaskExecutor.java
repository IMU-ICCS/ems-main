  ...
  public void create(PortProvided portProv) {
    ...
    Location locationEntity = configApi.getLocation(cloudId, location)
      .orElseThrow(() -> new IllegalArgumentException(format(
        "Location %s in cloud %s (id=%s) does not exist in Colosseum - port provided cannot be created", location, cloudName, cloudId)));

    Hardware hardwareEntity = configApi.getHardware(cloudId, hardware)
      .orElseThrow(() -> new IllegalArgumentException(format(
        "Hardware %s in cloud %s (id=%s) does not exist in Colosseum - port provided cannot be created", hardware, cloudName, cloudId)));

    Image imageEntity = configApi.getImage(cloudId, image)
      .orElseThrow(() -> new IllegalArgumentException(format(
        "Image %s in cloud %s (id=%s) does not exist in Colosseum - port provided cannot be created", image, cloudName, cloudId)));
    ...
    LifecycleComponent lcComponent = context.getLifecycleComponent(lcName)
      .orElseThrow(() -> new IllegalStateException(format(
        "Lifecycle component %s does not exist in Colosseum - port provided cannot be created", lcName)));
    ...
    VirtualMachineTemplate vmEntity = context.getVirtualMachine(cloudId, locationId, hardwareId, imageId)
      .orElseThrow(() -> new IllegalStateException(format(
        "Virtual Machine %s (cloudId=%s, locationId=%s, hardwareId=%s, imageId=%s) does not exist in Colosseum " +
          "- port provided cannot be created", vmName, cloudId, locationId, hardwareId, imageId)));
    ...
    ApplicationComponent acEntity = context.getApplicationComponent(appName, lcId, vmId)
      .orElseThrow(() -> new IllegalStateException(format(
        "Application component %s (appName=%s, lcId=%s, vmId=%s) does not exist in Colosseum " +
          "- port provided cannot be created", acName, appName, lcId, vmId)));
