...
#sudo
 if [ "$USECASE" == "FCR" ]
  ...
  then
    ...
    ./mule start "-M-Dhost.rulescaleout=select\ avg\(metricValue\)\ as\ metricValue,\ vmName\ as\ vmName,\ cloudName\ as\ cloudName,\ componentName\ as\ componentName,\ 2\ as\ level,\ current_timestamp\ as\ timestamp\ from\ Visor_event.win:time\(60\ sec\)\ having\ avg\(\metricValue\)\>80\ output\ last\ every\ 120\ sec" "-M-Dhost.rulescalein=select\ avg\(metricValue\)\ as\ metricValue,\ vmName\ as\ vmName,\ cloudName\ as\ cloudName,\ componentName\ as\ componentName,\ 2\ as\ level,\ current_timestamp\ as\ timestamp\ from\ Visor_event.win:time\(60\ sec\)\ having\ avg\(\metricValue\)\<\=50\ output\ last\ every\ 120\ sec" "-M-Dhost.ruleram=select\ avg\(metricValue\)\ as\ metricValue,\ vmName\ as\ vmName,\ cloudName\ as\ cloudName,\ componentName\ as\ componentName,\ 2\ as\ level,\ current_timestamp\ as\ timestamp\ from\ Visor_event.win:time\(60\ sec\)\ output\ last\ every\ 30\ sec"
    echo 'Mule started.'
  else
