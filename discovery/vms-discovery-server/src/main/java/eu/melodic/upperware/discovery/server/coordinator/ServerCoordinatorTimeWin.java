...
public class ServerCoordinatorTimeWin implements ServerCoordinator {
	...
	public ServerCoordinatorTimeWin(long registrationWindow, Runnable callback) {
	...
	protected synchronized void startPhase1(Vector<ClientShellCommand> registeredIntime) {
		...
		try {
			...
			if (brokerIpAddress==null || brokerIpAddress.trim().isEmpty() || brokerPort<=0) throw new Exception("ServerCoordinatorTimeWin: startPhase1(): Unable to get broker IP address or Port: "+broker);
