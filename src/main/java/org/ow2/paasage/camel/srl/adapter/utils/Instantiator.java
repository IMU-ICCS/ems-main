...
public class Instantiator {
    public static MetricInstance createMetricInstance(CamelModel model, EList<EObject> resourceContents, MetricContext context, MetricObjectBinding mob, EList<MetricInstance> metricInstances) {
        ...
        if(mob instanceof MetricComponentBinding){
            ...
            metricName += "_" + (mob.getExecutionContext().getApplication() == null? "" : mob.getExecutionContext().getApplication().getName()); // TODO PROBLEM - NO INSTANCE!
        }
        ...
        if (context.getMetric() instanceof RawMetric){
            ...
            if (sensor != null) MI.setSensor(sensor);
...
//                }
                        ...
                        for(MetricInstance mi : getMetricInstancesToContext(model, mc)){
                            if(mi.getMetricContext().equals(mc)){
                                ...
                                if (mi.getObjectBinding() instanceof MetricVMBinding) {
                                    mob = SingleFactory.getOrCreateNewMetricVMBinding(model, ec, resourceContents, ((MetricVMBinding)mi.getObjectBinding()).getVmInstance(), context.getApplication());
                                } else if(mi.getObjectBinding() instanceof MetricApplicationBinding){
                                    mob = SingleFactory.getOrCreateNewMetricApplicationBinding(model, ec, resourceContents, context.getApplication());
                                } else { //else if(mi.getObjectBinding() instanceof MetricComponentBinding){
                                    mob = SingleFactory.getOrCreateNewMetricComponentBinding(model,
                                            ec,
                                        resourceContents,
                                        ((MetricComponentBinding) mi.getObjectBinding())
                                            .getComponentInstance(),
                                        ((MetricComponentBinding) mi.getObjectBinding()).getVmInstance(),
        ...
        for(MetricInstance instance : model.getMetricModels().get(0).getMetricInstances()){
            if ((instance.getMetricContext() instanceof CompositeMetricContext)) {
                CompositeMetricContext compContext = (CompositeMetricContext)instance.getMetricContext();
                for(MetricContext metricContext : compContext.getComposingMetricContexts()){
                    ...
                    for (MetricInstance mi : camelFinder.getAllMetricInstancesToContext(metricContext)){
    ...
    public static EList<MetricInstance> getMetricInstancesToContext(CamelModel model, MetricContext mc){
        ...
        for(MetricInstance mi : model.getMetricModels().get(0).getMetricInstances()){
            if(mi.getMetricContext() != null && mi.getMetricContext().getName().equals(mc.getName())){
