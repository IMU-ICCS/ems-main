...
/*					boolean status = ((CommunicationAction) obj).getActionDone());
		...
		} else if(task.getTaskType()==TaskType.UPDATE){//updates with the new CAMEL id generated by S2D for the VMInstance in the new Deployment Model
			
			
			this.vmInstName = objParams.get("name").asString();
			String vmInstOldName = objParams.get("old_name").asString();
			LOGGER.log(Level.INFO, "VM Instance action (updation) thread : name " + vmInstName);
			
			String vmType = objParams.get("type").asString();//getting camel name for vm type
			String vmt;			
			
			if((vmt=dataShare.getEntityVMTid(vmType))!=null && dataShare.getEntityVMInstance(vmInstOldName)!=null){//verifying that the entities are existant in ExecWare
				
				if(dataShare.updateEntityVMInstance(vmInstOldName, this.vmInstName)){
					LOGGER.log(Level.INFO, "VM Instance name " + vmInstOldName + " changed to " + this.vmInstName);
				} else
					LOGGER.log(Level.WARNING, "VM Instance name " + vmInstOldName + " could not be changed to " + this.vmInstName);
			}
...
/*			//Forcing the CommunicationAction to run before deleting VMType but after deleting its VMInstances
	...
	/**
	 * The old name is available only when there's a {@link TaskType.UPDATE} action associated to it
	 * @return old name (if available) or else current name
	 */
	public String getOldVMInstName(){
		if(this.objParams.get("old_name") != null)
			return this.objParams.get("old_name").asString();
		else
			return this.vmInstName;
	}
