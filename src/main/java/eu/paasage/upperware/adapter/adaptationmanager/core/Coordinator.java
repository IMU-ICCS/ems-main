...
public class Coordinator {
	...
	private static DirectedGraph<Action, DefaultEdge> graph;//for Action dependencies
	...
	public static synchronized void initializeNeighbourDependencies(){
		synchronized (graph){
			...
			for(Action act : graph.vertexSet()){
				System.out.print("\nTasks dependent on " + act.toString() + " #" + neigh.successorsOf(act).size());
	...
	public static synchronized void setNeighbourDependencies(Action act){
		...
		for(Action pred : neigh.predecessorsOf(act)){
			if(holdDependents.containsKey(pred.toString())){
				...
				LOGGER.log(Level.WARNING, "PRED " + pred.toString() + " " + actionClassData);
			}
		}
	}
	
	public static synchronized Collection<Object> getNeighbourDependencies(Action act){
	...
	public static synchronized Collection<Action> getDependentOnActions(Action act){
	...
	public void runStep(){
		...
		DeploymentModel targetModel = reasonerInterfacer.getDeploymentModel(false);
...
/*		//Commenting for integrating Shirley's Plan Generator
		for (Action a : plan.getActions()){
			try {
				a.execute(inputMap, outputMap);
			} catch (ActionError e) {
				e.printStackTrace();
			}
...
//		DeploymentModel targetModel = reasonerInterfacer.getDeploymentModel(false);//Commented to replace live transaction below
		...
		DirectedGraph<Action, DefaultEdge> g = GraphUtilities.taskGraphToActions(taskPlan, execInterfacer);
		
/*		//GENERATING DEPENDENCIES OF ACTIONS AS A GRAPH
		DirectedGraph<Action, DefaultEdge> g = planGenerator.generatePlanGraph(currentModel, targetModel);
...
//			throw new AssertionError();*/
	...
	private static void schedule(){
		...
		synchronized (graph){
			...
			GraphIterator<Action, DefaultEdge> iterator = new DepthFirstIterator<Action, DefaultEdge>(graph);
...
//			executor.allowCoreThreadTimeOut(true);
		...
		synchronized (graph){
			...
			if(count == 0){
				...
				for(DefaultEdge edge : graph.outgoingEdgesOf(task)){
					System.out.print(" " + graph.getEdgeTarget(edge).toString());
				}
				return;
			}
		}
	...
	private static boolean deleteTask(Action task){
		...
		synchronized (graph){
			//System.out.println("Deleting task " + task.getClass() + " from graph. Id: " + task.toString());
			LOGGER.log(Level.INFO, "Deleting task " + task.getClass() + " from graph. Id: " + task.toString());
