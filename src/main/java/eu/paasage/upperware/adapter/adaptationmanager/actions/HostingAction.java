...
public class HostingAction implements Action {

	public HostingAction(DeploymentModel targetModel) {
		this.hostingName = (String) targetModel.eGet(targetModel.eClass()
				.getEStructuralFeature("name"));
	}
	
	public HostingAction(ConfigurationTask task, ExecInterfacer execInterfacer){
		this.task = task;
	...
	private static final Logger LOGGER = Logger
			.getLogger(HostingAction.class.getName());
	...
	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		...
		try {
			applicationId = execInterfacer.createApplication(hostingName);
		} catch (Exception e) {
	...
	public void run(){
		...
		if(task.getTaskType()==TaskType.CREATE){
			...
			this.hostingName = objParams.get("name").asString();
			LOGGER.log(Level.INFO, "Hosting Type action thread : name " + hostingName);
			
			//test for passing name parameter to Application Instance
			LOGGER.log(Level.INFO, this.hostingName);
