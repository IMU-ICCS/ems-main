...
public class ApplicationInstallationAction implements Action {
	private static final Logger LOGGER = Logger
			.getLogger(ApplicationInstallationAction.class.getName());

	public void execute(Map<String, Object> input, Map<String, Object> output) throws ActionError {
		LOGGER.log(Level.INFO, "Application installation action");
		ExecInterfacer execInterfacer = (ExecInterfacer) input
				.get("execInterfacer");
		String applicationId = (String) input.get("applicationID");
		try {
			execInterfacer.installApplication(applicationId);
		} catch (Exception e) {
			throw new ActionError();
		}
		output.putAll(input);
		output.put("installed", true);
	}
	
	public void run(){
		try{
			LOGGER.log(Level.INFO, "Application installation action thread");
