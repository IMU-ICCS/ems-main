...
public final class MetricsFinder {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MetricsFinder.class);

    private MetricsFinder() {
    }

    /**
     * Searches for the metric with the given metricName in the collection of metrics (parameter metrics) and returns the stored value if successful.
     * If the metric does not exist it returns null.
     */
    public static Object findMetric(Metrics metrics, String metricName) {
        ...
        LOGGER.info("found value for '{}' after Gauges: {}", metricName, value);

        if(value == null) {
            Counters counters = metrics.getCounters();
            value = findValueInCounters(counters, metricName);
            LOGGER.info("found value for '{}' after Counters: {}", metricName, value);
        }

        if(value == null) {
            LOGGER.info("No data found for {}", metricName);
        }
        return value;
    }
    ...
    private static Object findValueInProperties(Map<String, Object> propertyMap, String metricName, String valuePropertyName) {
