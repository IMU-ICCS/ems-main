...
public final class FileSystemShellUtils {
	...
	/**
	 * Removes {@link Constants#HEADER} / {@link Constants#HEADER_FT} and
	 * hostname:port information from a path, leaving only the local file path.
	 *
	...
	/**
	 * Validates the path, verifying that it contains the {@link Constants#HEADER}
	 * or {@link Constants#HEADER_FT} and a hostname:port specified.
	 *
	 * @param path the path to be verified
	 * @return the verified path in a form like alluxio://host:port/dir. If only the
	 *         "/dir" or "dir" part is provided, the host and port are retrieved
	 *         from property, alluxio.master.hostname and alluxio.master.port,
	 *         respectively.
	 */
	...
	/**
	 * Gets all the {@link AlluxioURI}s that match inputURI. If the path is a
	 * regular path, the returned list only contains the corresponding URI; Else if
	 * the path contains wildcards, the returned list contains all the matched URIs
	...
	public static List<AlluxioURI> getAlluxioURIs(FileSystem alluxioClient, AlluxioURI inputURI) throws IOException {
		if (!inputURI.getPath().contains(AlluxioURI.WILDCARD)) {
			...
			AlluxioURI parentURI = new AlluxioURI(inputURI.getScheme(), inputURI.getAuthority(),
					inputPath.substring(0, inputPath.indexOf(AlluxioURI.WILDCARD) + 1), inputURI.getQueryMap())
							.getParent();
	 ...
	 * until it finds all the matches; It does not go into a directory if the prefix
	 * mismatches (e.g., for input "/a/b/*", it won't go inside directory "/a/c")
	 *
	 * @param alluxioClient the client used to fetch metadata of Alluxio files
	 * @param inputURI      the input URI (could contain wildcards)
	 * @param parentDir     the {@link AlluxioURI} of the directory in which we are
	 *                      searching matched files
	 * @return a list of {@link AlluxioURI}s of the files that match the inputURI in
	 *         parentDir
	 */
	private static List<AlluxioURI> getAlluxioURIs(FileSystem alluxioClient, AlluxioURI inputURI, AlluxioURI parentDir)
			throws IOException {
	...
	/**
	 * Gets the files (on the local filesystem) that match the given input path. If
	 * the path is a regular path, the returned list only contains the corresponding
	 * file; Else if the path contains wildcards, the returned list contains all the
	 * matched Files.
	 *
	...
	/**
	 * Gets all {@link Command} instances in the same package as
	 * {@link FileSystemShell} and load them into a map. Provides a way to gain
	 * these commands information by their CommandName.
	 *
	...
	public static Map<String, Command> loadCommands(FileSystem fileSystem) {
		return CommandUtils.loadCommands(FileSystemShell.class.getPackage().getName(), new Class[] { FileSystem.class },
