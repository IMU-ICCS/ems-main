...
rem java %JAVA_OPTS% -jar %BROKER_CEP_TEST% control-service\target\control-service.jar --logging.config=file:${MELODIC_CONFIG_DIR}/logback-conf/logback-spring.xml
rem Use when Esper is NOT packaged in control-service.jar
java %JAVA_OPTS% -Djasypt.encryptor.password=%JASYPT_PASSWORD% -cp control-service\target\control-service.jar -Dloader.path=control-service\target\esper-7.1.0.jar %BROKER_CEP_TEST% org.springframework.boot.loader.PropertiesLauncher -nolog --logging.config=file:${MELODIC_CONFIG_DIR}/logback-conf/logback-spring.xml
