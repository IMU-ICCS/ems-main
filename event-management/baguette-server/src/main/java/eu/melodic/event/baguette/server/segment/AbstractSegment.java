...
public abstract class AbstractSegment implements Segment {
	...
	public static Set<String> getSegmentIds() { return segments.keySet(); }
	public static Collection<Segment> getSegments() { return segments.values(); }
	...
	public String getState() { return fsm.getState(); }
	...
	private static class QueueEvent {
		String event;
		Object[] args;
		QueueEvent(String e, Object[] a) { event = e; args = a; }
	}
	...
	protected void sendEvent(String event, Object... args) {
		...
		try { eventQueue.put(new QueueEvent(event, args)); } catch (InterruptedException ex) { log.warn("sendEvent: Waiting to enqueue event interrupted"); }
	}
			...
			new Runnable() {
				public void run() {
					...
					while (keepRunning || eventQueue.size()>0) {
						...
						} catch (InterruptedException ex) { log.warn("EventDispatcher: Waiting for event interrupted"); }
					}
				}
			}
		);
	...
	private void stopEventDispatcher() {
		keepRunning = false;
		try { eventDispatcher.interrupt(); } catch (Exception ex) {}
