...
public class BaseScenarioSegment extends AbstractSegment {
	...
	public long getTimeoutPeriod() { return timeoutPeriod; }
	public int getExpectedMembers() { return expectedMembers; }
	public void setTimeoutPeriod(long t) { checkNotStarted(); timeoutPeriod = t; }
	public void setExpectedMembers(int n) { checkNotStarted(); expectedMembers = n; }
	public SELECTION_METHOD getSelectionMethod() { return brokerSelectionMethod; }
	public void setSelectionMethod(SELECTION_METHOD sm) { checkNotStarted(); brokerSelectionMethod = sm; }
	public Function getReadyCallback() { return readyCallback; }
	public void setReadyCallback(Function f) { readyCallback = f; }
	public Function getChangeCallback() { return changeCallback; }
	public void setChangeCallback(Function f) { changeCallback = f; }
	
	public Properties getBrokerConfig() { return brokerConfig; }
	public void setBrokerConfig(Properties p) { brokerConfig = p; }
	public Properties getClientConfig() { return clientConfig; }
	public void setClientConfig(Properties p) { clientConfig = p; }
	public boolean getNoBroker() { return noBroker; }
	public void setNoBroker(boolean b) { noBroker = b; }
	...
	public synchronized void start() {
		checkNotStarted();
		if (!noBroker && brokerSelectionMethod==SELECTION_METHOD.UNDEFINED) throw new IllegalStateException("Broker selection method has not been set for segment "+id);
		if (!noBroker && (brokerConfig==null || brokerConfig.size()==0)) throw new IllegalStateException("Broker configuration has not been set or is invalid, for segment "+id);
		if (noBroker && (clientConfig==null || clientConfig.size()==0)) throw new IllegalStateException("Client configuration has not been set or is invalid, for segment "+id);
		super.start();
		startTimeout();
	...
	public synchronized void brokerReady(ClientShellCommand broker) {
		...
		if (this.broker!=broker) { log.warn("brokerReady: Argument does not match current broker: arg={}, curr={}", broker, this.broker); return; }
		if (this.broker==null) { log.warn("brokerReady: No broker set: broker={}", this.broker); return; }
	...
	private synchronized Object registerPending(Object args) {
		...
		if (timeoutPeriod<=0 && expectedMembers<=0) {
		...
		if (changeCallback!=null) {
			...
			Thread thread = new Thread(
				new Runnable() {
					...
					public Runnable setSegment(Segment s) { seg = s; return this; }
				}
				.setSegment(this)
			);
	...
	public synchronized ClientShellCommand getBroker() { return broker; }
	private synchronized void setBroker(ClientShellCommand csc) { if (!noBroker) broker = csc; else broker = null; }
