...
public class CloudiatorInitialiazer {
    ...
    public void deployApplication(int target) {
        if (target < 0) {
            log.warn("Nothing to do: target={}", target);
            return;
        }
        int step = 0;
        ...
        if (step++ < target)
            createMultipleEntities(LifecycleComponent.class, "Lifecycle Components", "comp", this::createLcComponent);
        if (step++ < target) createApplicationInstance();
        if (step++ < target)
            createMultipleEntities(VirtualMachineTemplate.class, "Virtual Machine Template", "vmt", this::createVirtualMachineTemplate);
        if (step++ < target)
            createMultipleEntities(ApplicationComponent.class, "Application Component", "ac", this::createApplicationComponent);
        if (step++ < target)
            createMultipleEntities(VirtualMachine.class, "Virtual Machine", "vm", this::createVirtualMachine);
        if (step++ < target)
            createMultipleEntities(PortProvided.class, "Provided Port", "provided", this::createPortProvided);
        if (step++ < target)
            createMultipleEntities(PortRequired.class, "Required Port", "required", this::createPortRequired);
        if (step++ < target)
            createMultipleEntities(Communication.class, "Communication", "comm", this::createCommunication);
    ...
    protected boolean checkProviderOffers(boolean throughException) {
        ...
        if (countImages == 0 || countHardware == 0 || countLocations == 0) {
            if (throughException)
                throw new RuntimeException("Cloudiator has not been initialized with Provider Offers");
            else return false;
        }
    ...
    protected Long getIdByName(List<? extends RemoteEntityInCloud> list, String name) {
        for (RemoteEntityInCloud re : list) {
            ...
            } else if (re instanceof Location) {
    ...
    protected void createMultipleEntities(Class clazz, String label, String configDomain, java.util.function.Function<Integer, Object> callback) {
        int n;
        if ((n = countEntitiesOf(clazz)) > 0) {
            log.info("{}s have already been configured: count={}", label, n);
            return;
        }
    ...
    protected void createApi() {
        ...
        if (apiId > 0) {
            log.info("API pre-configured: id={}", apiId);
            return;
        }
    ...
    protected void createCloud() {
        ...
        if (cloudId > 0) {
            log.info("Cloud pre-configured: id={}", cloudId);
            return;
        }
    ...
    protected void createCloudCredentials() {
        ...
        if (credentialsId > 0) {
            log.info("Cloud Credentials pre-configured: id={}", credentialsId);
            return;
        }
    ...
    protected void createApplication() {
        ...
        if (appId > 0) {
            log.info("Application pre-configured: id={}", appId);
            return;
        }
    ...
    protected void createApplicationInstance() {
        ...
        if (appInstanceId > 0) {
            log.info("Application Instance pre-configured: id={}", appInstanceId);
            return;
        }
