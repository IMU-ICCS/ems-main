...
public class GeneticSolverCoordinator {
    ...
    public void generateCPSolutionFromFile(String applicationId, String cpModelFilePath, String nodeCandidatesFilePath) {
        try {
            ...
            cpWrapper.parse(cp, new UtilityProviderImpl(utilityGenerator));
            CPGeneticWrapper wrapper = new CPGeneticWrapper(cpWrapper);
            List<Integer> result = solver.run(wrapper);
