...
public class NCSolverControllerImpl implements SolverController {
    ...
    @Override
    public String solve(NodeCandidates nodeCandidates, ConstraintProblem cp, UtilityProvider utility, String cpId) {
        log.info("Starting " + SOLVER_ID + " on " + cpId);
        NCSolver solver = new NCSolver(ptParameters.getMinTmp(), ptParameters.getMaxTmp(), ptParameters.getNumThreads(), cp, utility, nodeCandidates);
