...
//	}
//
//	public static Optional<DeploymentModel> getLastDeployedTypeModel(String camelModelID, CDOTransaction transaction){
//		CamelModel camelModel = getLastCamelModel(transaction.getResource(camelModelID).getContents())
//				.orElseThrow(() -> new IllegalStateException("Could not find camel model from camelModelID: " + camelModelID));
//
...
//
//			((CamelModel)transactionManager.deploymentTypeModel.eContainer()).getProviderModels().addAll(dataHolder.getProviderModel());
//			transactionManager.deploymentTypeModel.getInternalComponentInstances().addAll(dataHolder.getComponentInstancesToRegister());
//			transactionManager.deploymentTypeModel.getVmInstances().addAll(dataHolder.getVmInstancesToRegister());
//			transactionManager.deploymentTypeModel.getHostingInstances().addAll(dataHolder.getHostingInstancesToRegister());
//			transactionManager.deploymentTypeModel.getCommunicationInstances().addAll(dataHolder.getCommunicationInstances());
//
...
//
//			DeploymentModel deploymentTypeModel;
...
//						.orElseThrow(() -> new IllegalStateException("Could not find camel model from camelModelID: " + camelModelID));
//				this.deploymentTypeModel = camelModel.getDeploymentModels().get(dmId);
//				this.dmId = dmId;
//			}
//
//			void commit() {
