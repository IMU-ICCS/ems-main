    ...
    public DeploymentResponse createDeploymentProcess(DeploymentProcessRequest deploymentProcessRequest) {
        ...
        try {
            ...
            if (processResponse.getBody() != null && processResponse.getBody().getProcessAuthorizationStatus() != 200) {
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, processResponse.getBody().getProcessAuthorizationResponse().get(0).get("message"));
            }

            // Cloudiator service not working
            if (processResponse.getBody() != null && processResponse.getBody().getFeedCloudiatorResponse() == null) {
            ...
            // todo invalid provider credentials response (after Jira MEL-754)
