...
public class BaguetteClient implements ApplicationRunner {
    ...
    private final List<Class<? extends ClientCollector>> DEFAULT_COLLECTORS_LIST = List.of(
        K8sNetdataCollector.class, PrometheusCollector2.class
    );

    @Getter
    private final List<ClientCollector> collectorsList = new ArrayList<>();
    ...
    protected void startCollectors() {
        ...
        for (Class<? extends ClientCollector> collectorClass : baguetteClientProperties.getCollectorClasses()) {
            try {
                log.debug("BaguetteClient: Starting collector: {}...", collectorClass.getName());
                ClientCollector collector = applicationContext.getBean(collectorClass);
    ...
    protected void stopCollectors() {
        ...
        for (ClientCollector collector : collectorsList) {
            try {
