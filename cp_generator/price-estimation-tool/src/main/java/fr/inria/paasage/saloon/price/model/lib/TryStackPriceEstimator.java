...
public class TryStackPriceEstimator implements IProviderPriceEstimator 
{
	...
	{
		logger.debug("TryStackPriceEstimator - computeVmsPrice- Computing the price... ");
		double price= 0; 
		...
		logger.debug("TryStackPriceEstimator - computeVmsPrice- Computing the price for vm: "+((EnumerateValue)vmType.getValue()).getName());
		...
		if(vmsMap.get(n1)!=null)
		{
			...
			logger.warn("TryStackPriceEstimator - computeVmsPrice- The rate for vm: "+((EnumerateValue)vmType.getValue()).getName()+ "cannot be found. The default value will be used");
	...
	public void loadLocationRates(BufferedReader br)
	{
		try 
		{
			...
			logger.debug("TryStackPriceEstimator - loadLocationRates - processing line "+line);
			...
			if(!line.startsWith("#"))
			{
				...
				if(infos.length==2)
				{
					...
					logger.error("TryStackPriceEstimator - loadLocationRates - The line "+line+" does not have the correct format. The price will be not loaded!"); 
			}
