...
public class CamelModelProcessorService {
    ...
    //    /**
//     * Gets elements from {@link #deploymentModelParser} and fills the specified pc.
//     * @param pcw The PaaSage Configuration to be filled
//     * @param preferedProviders
//     */
//    public void parseModel(CamelModel model, PaaSageConfigurationWrapper pcw, Set<String> preferedProviders) {
//
//        logger.debug("CamelModelProcessor - parseModel - Calling DeploymentModelParser!");
//        logger.info(" ** 	Calling DeploymentModelParser");
//        DeploymentModel deploymentModel = model.getDeploymentModels().get(0);
//        deploymentModelParser.parseDeploymentModel(deploymentModel, pcw);
//
//        //Map<String,List<VirtualMachineProfile>> vmProfiles= deploymentModelParser.getVmProfiles();
//
//        List<VM> vms=  DeploymentModelParser.getVMList(deploymentModel);
//        logger.info(" ** 	Processing Opt Rerqs");
//        parseOptimisationRequirements(model, pcw);
//        logger.info(" ** 	Processing Opt Rerqs ended");
//        VMRequirementSet globalRequirements= deploymentModel.getGlobalVMRequirementSet();
//
//        //TODO - poczatek
//        EList<RequirementModel> requirementModels = model.getRequirementModels();
//        EList<Requirement> requirements = requirementModels.get(0).getRequirements();
//
//
//
...
//                        providerModelParser.parseOntology(ontology, pcw, pmWithImage, vm, currentCandidates); //TODO CLEAN OR RELOAD THE PROVIDER MODELS ?????
...
//                            providerModelParser.removeCandidatesWithLocationForVM(vm,pmWithImage.getProviderId(),concreteLocationConcept.getName(),pcw);
//                        }
//                        else
...
//                        processProviderRequirementsLocation(provReq, ontology, pcw, vm, candidates, concreteLocationConcept.getName());
...
//                        processAllProvidersLocation(ontology, pcw, vm, candidates,concreteLocationConcept.getName());
...
//            {
//                providerModelParser.parseOntology(ontology, pcw, pmWithImage, vm,candidates); //TODO CLEAN OR RELOAD THE PROVIDER MODELS ?????
//            }
//            else if(provReq!=null) //We only consider the specified providers
//            {
//                processProviderRequirements(provReq, ontology, pcw, vm, candidates);
//            }
//            else //We have to process all the providers
//            {
//                logger.debug("CamelModelProcessor - parseModel - processAllProviders");
//
//                processAllProviders(ontology, pcw, vm, candidates);
//            }
//
//        }
//
//        providerModelParser.removeNotPreferedProviders(preferedProviders, pcw.getPaasageConfiguration());
//
//        providerModelParser.removeNoCandidateProviders(pcw.getPaasageConfiguration(), candidates);
//
//        providerModelParser.checkExistSolution(pcw);
//
//        logger.debug("CamelModelProcessor - parseModel - Checking solution existency ");
//        deploymentModelParser.checkExistencyOfValidUserSolution(deploymentModel, pcw);
...
//        deploymentModelParser.checkCorrectHostingRelationships(deploymentModel, pcw);
...
//                }
//            }
//        } else if (osImageReq instanceof ImageRequirement) {
//            ImageRequirement imgReq= (ImageRequirement) osImageReq;
//            String imageId= imgReq.getImageId();
//
//            for (ProviderModelDecorator providerModelDecorator : proxy.loadPMs().values()) {
//                Feature root= providerModelDecorator.getRootFeature();
//
//                Attribute att= SaloonCamelSolver.getEnumAttributeWithValueInDomain(root, imageId);
//                if(att!=null) {
//                    pmWithImage = providerModelDecorator;
//                }
//            }
//        }
//        return pmWithImage;
...
//        {
//            ProviderModel pm = proxy.loadPMs().get(prov.getName());
//
//            if(pm!=null) {
////                providerModelParser.parseOntology(ontology, pc, pm, vm, candidates); //TODO -
//            } else {
...
//        for(String key: proxy.loadPMs().keySet())
//        {
//
//            ProviderModelDecorator pm= proxy.loadPMs().get(key);
...
//    }
//
//    protected void parseOptimisationRequirements(CamelModel model, PaaSageConfigurationWrapper pc) {
