...
/config/wait-for-cdo.sh && java  -Djasypt.encryptor.password=${JASYPT_PASSWORD} -Djavax.net.ssl.trustStore=/config/common/melodic-truststore.p12 -Djavax.net.ssl.trustStorePassword=melodic -Djavax.net.ssl.trustStoreType=pkcs12 -Djava.security.egd=file:/dev/./urandom -jar meta-solver.jar "--logging.config=file:$LOG_CONFIG_FILE"
